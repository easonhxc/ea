...
export function validatePlan(plans=[]) {
  ...
  if (restrictive.length) {
    ...
    const conflicts=plans.filter(p=>p!==anchor && p.country==="us" && ["ED1","EA","EA2","REA","SCEA"].includes(p.round));
...
function draw(p) {
  ...
  return Math.random()<Math.min(.999,probability);
}
...
export function monteCarlo(plans=[],runs=1000) {
  ...
  let total=0,zero=0,binding=0,top20=0,top30=0;
  let visible=null;
  ...
  for (let i=0;i<runs;i++) {
    ...
    if (cycle.results.some(r => (r.rank||999)<=30 && (r.outcome==="Admit"||r.outcome==="Defer → Admit"))) top30++;
  }
