...
function prefScore(profile,name){
  const norm=v=>String(v||"").toLowerCase().replace(/[^a-z0-9]/g,"");
  const n=norm(name);const row=(profile?.school_preferences||[]).find(x=>{const q=norm(x.school_name);return q&&(q===n||q.includes(n)||n.includes(q))});
