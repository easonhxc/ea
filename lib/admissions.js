...
function highSchoolContext(profile,target,highSchoolOverrides=[]){
  ...
  const offerHistory=(hs.outcomes?.offer_targets||[]).some(x=>x.toLowerCase()===target.name.toLowerCase());
  if(direct)boost+=.01;
  else if(offerHistory)boost+=.005;
  ...
  return {multiplier:1+boost,label:`Verified school context +${(boost*100).toFixed(1)}% relative`,source:hs.outcomes_url||hs.profile_url,school:hs.name,year:hs.outcome_year,direct,offer_history:offerHistory};
}
...
export function predict(profile,primaryMajorText,secondaryMajorText,overrides=[],aiAssessment=null,highSchoolOverrides=[]){
  ...
  const primaryInput=primaryMajorText||profile.primary_major||"";
  const secondaryInput=secondaryMajorText||profile.secondary_major||"";
  const primaryResolution=resolveMajor(primaryInput);
  const secondaryResolution=secondaryInput?resolveMajor(secondaryInput):{slug:null,label:null,matched:false,method:"empty",confidence:0,category:"other"};
  const primarySlug=primaryResolution.slug, secondarySlug=secondaryResolution.slug;
  const unresolvedAll=!primarySlug&&!secondarySlug;
  const primaryScore=scoreProfile(profile,primaryResolution.label||primaryInput||"Undeclared",aiAssessment);
  const secondaryScore=secondarySlug?scoreProfile(profile,secondaryResolution.label||secondaryInput,aiAssessment):null;
  const rows=[];
  const warnings=[];
  if(primaryInput&&!primaryResolution.matched) warnings.push(`Primary major “${primaryInput}” was not matched to the major catalog; using a broad undeclared fallback instead of returning an empty list.`);
  if(secondaryInput&&!secondaryResolution.matched) warnings.push(`Secondary major “${secondaryInput}” was not matched to the major catalog and is not used for program filtering.`);
  if(!primaryInput&&!secondaryInput) warnings.push("No intended major is selected, so results use a broad undeclared planning model. Select a major for program-level matching.");

  for(const s of schools){
    const has1=primarySlug?s.support.includes(primarySlug):unresolvedAll;
    const has2=secondarySlug?s.support.includes(secondarySlug):false;
    if(!has1&&!has2)continue;
    let p1=has1?probability(s,profile,primaryScore,highSchoolOverrides):null;
    let p2=has2?probability(s,profile,secondaryScore,highSchoolOverrides):null;
    let chosen=has1?"primary":"secondary",reason=null;
    if(p1&&p2){
      if(SCHOOLWIDE.has(s.name)){p2={...p2,center:p1.center,min:p1.min,max:p1.max};reason="Schoolwide admission: intended-major switching does not create a modeled boost."}
      else if(p2.center>=Math.max(p1.center+.025,p1.center*1.18)){chosen="secondary";reason="Secondary direction is modeled as materially friendlier; only use it if the profile genuinely supports that field."}
      else reason="Primary direction remains the stronger strategic choice.";
    }
