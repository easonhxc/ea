...
import { buildApplicantIntelligence } from "@/lib/applicant-intelligence";
...
function probabilityEvidenceConfidence(profile,s,score){
  let c=.43;
  if(profile.grade_band&&profile.grade_band!=="unknown")c+=.08;
  if(profile.gpa_description)c+=.05;
  if(profile.sat||profile.act)c+=.06;
  if((profile.ap_courses?.length||0)+(profile.ib_courses?.length||0)+(profile.alevel_courses?.length||0)>=4)c+=.08;
  if((profile.activities||[]).length>=4)c+=.05;
  if((profile.activities||[]).filter(a=>(a.description||"").length>=80).length>=2)c+=.05;
  if((profile.activities||[]).filter(a=>a.measurable_outcome).length>=2)c+=.04;
  if((profile.distinctive_outputs||[]).length)c+=.03;
  if(score?.course_fit?.confidence>=.65)c+=.04;
  if(s.data_quality==="verified"||s.catalog_verified)c+=.04;
  return Math.max(.40,Math.min(.90,c));
}
function probability(s,profile,score,highSchoolOverrides=[]){
  ...
  const confidence=probabilityEvidenceConfidence(profile,s,score),label=confidence>=.78?"High":confidence>=.60?"Medium":"Low";
  const lowFactor=confidence>=.78?.76:confidence>=.60?.68:.58,highFactor=confidence>=.78?1.30:confidence>=.60?1.42:1.58;
  const min=Math.min(p,clamp(p*lowFactor,.004,ceiling)),max=Math.max(p,Math.min(ceiling,Math.max(p+.012,p*highFactor)));
  return {center:p,min,max,baseline,high_school_context:hs,confidence:Number(confidence.toFixed(2)),confidence_label:label,method:"Planning range derived from institutional baseline, applicant competitive signal, program fit and bounded context adjustments. It is not an admissions-office probability."}
}
